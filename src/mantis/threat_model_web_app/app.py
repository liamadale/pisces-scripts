...
from src.mantis.threat_model_web_app.data import (
    ALL_ATTACK_TYPES,
    ALL_BLOCKLISTS,
    ALL_FP_CATEGORIES,
    FP_BY_IP,
...
def _filter_tp(args) -> list[dict]:
    ...
    if min_tickets_str:
        try:
            ...
            rows = [r for r in rows if r["tickets"] >= min_t]
        except (ValueError, TypeError):
            pass
    return rows
...
def create_app() -> Flask:
    ...
    def api_ticket_detail(tid: str):
        ...
        if not t:
            return "<p style='color:var(--on-surface-dim);padding:1rem'>Ticket not found.</p>", 404
