...
def test_build_base_query_dataset_filter() -> None:
    ...
    must = body["query"]["bool"]["must"]
    dataset_clause = next(
        (c for c in must if "terms" in c and "event.dataset" in c["terms"]), None
    )
...
def test_build_base_query_sensor_filter() -> None:
    ...
    must = body["query"]["bool"]["must"]
    sensor_clause = next(
        (c for c in must if "terms" in c and "host.name" in c["terms"]), None
    )
...
def test_build_base_query_src_ip_filter() -> None:
    ...
    must = body["query"]["bool"]["must"]
    ip_clause = next(
        (c for c in must if "term" in c and "source.ip" in c["term"]), None
    )
